import { CATERORY_IMAGE_PLACEHOLDER } from "@/lib/constants"
...
function CollectionBanner({ url, bgUrl }: { url: string; bgUrl: string }) {
  return (
    <Link
      href={url}
      className="block relative w-full h-2/3 sm:w-1/2 lg:w-1/3 xl:w-1/4"
    >
      ...
      <Image
        width={400}
        height={50}
        className="w-full h-96 sm:h-[370px] object-cover object-center"
        src={bgUrl || CATERORY_IMAGE_PLACEHOLDER}
        alt={url}
      />
    </Link>
  )
