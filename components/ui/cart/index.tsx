...
const ProductCart: React.FC<ProductCartProps> = ({ name, url, price }) => {
  return (
    <figure className='relative py-5 md:py-0 h-[98%] rounded-md flex flex-col justify-between hover:shadow-xl max-w-full origin-top hover:scale-100 active:rotate-2 ease-in-out transition-all duration-1000'>
      <Image width={180} height={230} src={url} alt={name} className=' w-full max-w-full h-full object-cover bg-[#F6F6F6]' />
