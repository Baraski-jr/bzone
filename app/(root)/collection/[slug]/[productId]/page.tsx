...
const ProductDetail: React.FC<PageProps> = ({ params }) => {
  const pat = params.productId.replace(/-/g, ' '); // Replace dashes with spaces
  const [id, setId] = useState<number | null>(null); // Use `null` as the initial state
  const [product, setProduct] = useState<ProductsType | undefined>(); // Handle undefined state
  const [index, setIndex] = useState(0);
  const discount = 10;

  useEffect(() => {
    const filt = products.filter((items) => items.title === pat); // Find product by title
    if (filt.length > 0) {
      setProduct(filt[0]);
      setId(filt[0].id); // Set the product ID
    } else {
      setProduct(undefined); // Product not found
    }
  }, [pat]); // Include `pat` in the dependency array

  // If no product is found, render a fallback
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-7 md:gap-x-10">
          ...
          <div className="md:flex">
            ...
            <div className="md:hidden pt-5">
              <Thumbnails id={id || 0} images={product.images} setIndex={setIndex} />
            </div>
          </div>
          {/* Product details */}
          <div className="w-full flex flex-col gap-6">
            ...
            {/* Ensure products[id] is valid before accessing sizes */}
            <CustomizeProduct
              colours={product.colours}
              sizes={id !== null && products[id] ? products[id].sizes : []}
            />
            <div className="border-y-4 border-gray-50 py-3">
              {/* Control AddToCart button disable state */}
              {product.inventory === 0 ? (
            ...
            {/* Control Buy button disable state */}
